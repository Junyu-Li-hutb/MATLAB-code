function [fit,chromosome]=compute_fitness(chromosome,distance,Requirement,Servicetime,parameter,price,time_period,Chargingindex)
        fit.car=length(chromosome)*price.vehicle;
    fit.timecost=0;
            ...
            if ~isempty(index1)
                if index2<index1
                    for a=1:length(path)-1
                        ...
                        elseif nextNode==path(index1)
                            ...
                            fit.dischargeele=fit.dischargeele+Battery-total_eij2;
                            ...
                            fit.fangelecost=fit.fangelecost+(Battery-total_eij2)*pr1;
                            time=time+time_fang;
                            Battery=total_eij2;
                        else
                            if nowNode>200
                            ...
                            time=time+(distance(nowNode,nextNode)/parameter.v)/60;
                            timearrive(end+1)=time;
                            time=time+Servicetime(nextNode);
                        end
                    end
                    fit.timecost=fit.timecost+time*price.time;
                    fit.time{end+1}=timearrive;
                else
                    for a=1:length(path)-1
                        nowNode=path(a);nextNode=path(a+1);
                        if nextNode==path(index1)
                            ...
                            elseif time>=time_period(6)&&time<time_period(7)
                                if time+time_fang<=timge_period(7)
                                    V2G_shouyi=(Battery-total_eij2)*price.discharge2;
                                else
                                    fang1=((time_period(7)-time)/60)*parameter.pf;
                                    V2G_shouyi=fang1*price.discharge2+((Battery-total_eij2)-fang1)*price.discharge1;
                            ...
                            fit.fangelecost=fit.fangelecost+(Battery-total_eij2)*price.chargefire1;
                        ...
                        elseif nextNode==path(index2)
                            ...
                            fit.chargeele=fit.chargeele+total_eij1-Battery;
                            if time>time_period(1)&&time<time_period(2)
                                if tag==0
                ...
                for a=1:length(path)-1
                    ...
                    if nextNode==path(index2)
                        ...
                        if total_eij1-Battery<0
                            fit.charge_num=fit.charge_num-1;
                            fit.charge_begin(end)=[];
                            chromosome{i}(index2)=[];
                            continue
                        end
                        fit.chargeele=fit.chargeele+total_eij1-Battery;
            ...
            if ~isempty(index1)
               ...
               fit.time{end+1}=timearrive;
                ...
                for a=1:length(path)-1
                    ...
                    eij1=energy(parameter,Content,distance,nowNode,nextNode);
                    Battery=Battery-eij1;
                    Content=Content-Requirement(nextNode);time=time+(distance(nowNode,nextNode)/parameter.v)/60;
                    timearrive(end+1)=time;
                    time=time+Servicetime(nextNode);
                end
                fit.timecost=fit.timecost+time*price.time;
    ...
    fit.cost=fit.car+fit.timecost+fit.chargecost;
